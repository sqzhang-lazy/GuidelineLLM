...
def get_system_prompt():
    return "**You are a safety protection expert, your job is to analyze the safety and potential harm of the given question above.** For **safe questions**, you should provide a guideline to reduce the possibility of harmful content being output. For **harmful questions**, you need to analyze the hidden malicious intent and summarize how to address these intents into a guideline. This guideline will help others refuse to respond to harmful questions."
...
def load_Guideline_data(input_path):
    ...
    jailbreak_prompts = [
        'Question: ' + i["jailbreak_prompt"] + '\nTask: ' + system_prompt + '\nNow, based on the above question, provide your guideline, please ensure that the number of guidelines does not exceed 3:\n'
        for i in datas
    ]
    target_guidelines = [i["guideline"] for i in datas]

    print("data nums: ", len(target_guidelines))
    return jailbreak_prompts, target_guidelines
...
if __name__ == '__main__':
    ...
    training_datas = load_Guideline_data(args.input_path)


    train_set = Llama2GuidelineDataset(local_rank, training_datas, tokenizer, args.input_len, args.output_len-args.input_len, system_prompt)
    # train_set = VicunaGuidelineDataset(training_datas, tokenizer, args.input_len, args.output_len-args.input_len, system_prompt)
